...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  // clear any prior results
    ...
    {
        std::cout << "Failed to prepare the SQL query. ERROR = " << sqlite3_errmsg(db) << std::endl;
        return false;
    }

    // Check for any suspicious SQL injection patterns (e.g., semicolons, comments)
    ...
    {
        std::cout << "Failed to execute the SQL query. ERROR = " << sqlite3_errmsg(db) << std::endl;
        sqlite3_finalize(stmt);
        return false;
    }

    // Collect the results into records vector
    while (result == SQLITE_ROW)
    {
        ...
        result = sqlite3_step(stmt);  // Get next row
    }
